...
#define NDMaxSections 100
@interface NDBannerView ()<UICollectionViewDataSource ,UICollectionViewDelegate ,UICollectionViewDelegateFlowLayout>
...
/** cell的重用标识 */
static NSString *const NDBannerID = @"NDBannerViewCell";
...
- (UICollectionViewFlowLayout *)layout
{
    if (!_layout) {
        ...
        _layout.itemSize = CGSizeMake(self.xtfei_width, self.xtfei_height);
...
- (UICollectionView *)bannerCollectionView
{
    if (!_bannerCollectionView) {
        ...
        [_bannerCollectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:NDMaxSections * 0.5] atScrollPosition:UICollectionViewScrollPositionLeft animated:true];
...
- (void)registerCollectionViewCell
{
    ...
    [_bannerCollectionView registerNib:[UINib nibWithNibName:NSStringFromClass([NDBannerViewCell class]) bundle:nil] forCellWithReuseIdentifier:NDBannerID];
}
...
- (UIPageControl *)pageControl
{
    if (!_pageControl) {
        ...
        CGFloat pageControlY = self.xtfei_height - 10;
        CGFloat pageControlW = self.xtfei_width;
...
{
    return NDMaxSections;
}

- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    return self.banner.count ? self.banner.count : 1;
}

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    NDBannerViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NDBannerID forIndexPath:indexPath];
...
- (void)nextPage
{
    ...
    NSIndexPath *indexPathSet = [NSIndexPath indexPathForItem:indexPath.row inSection:NDMaxSections/2];
...
{
    if (self.bannerClick) {
        self.bannerClick(self, _banner[indexPath.row].image_path);
    }
}
...
- (void)bannerViewClick:(void (^)(NDBannerView *, NSString *))block
{
