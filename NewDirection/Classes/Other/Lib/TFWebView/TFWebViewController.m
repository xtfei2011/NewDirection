...
- (void)rightBarButtonItem
{
    ...
    [roadButton setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
...
- (WKWebView *)webView
{
    if (!_webView) {
        ...
        _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, NDMainScreen_Width, NDMainScreen_Height) configuration:configuration];
        _webView.backgroundColor = NDRandomColor;
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
    ...
    //删除顶部的导航条
    [js1 appendString:@"var head = document.getElementsByClassName('header')[0];"];
    [js1 appendString:@"head.parentNode.removeChild(head);"];
...
- (UIProgressView *)progressView
{
    if (!_progressView) {
        ...
        _progressView.progressTintColor = NDColor(38, 123, 239);
    }
